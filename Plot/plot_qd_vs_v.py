...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot current vs time, voltage vs time, and discharge capacity vs voltage from a BatteryML .pkl file."
    )
    ...
    if all_files_mode:
        ...
        plot_three_panel_metric_dashboard(
            curves_by_subfolder,
            metric_title="Qdischarge vs Voltage",
            metric_ylabel="Qdischarge (Ah)",
    ...
    if combined_mode:
        ...
        for path, obj in objects:
            curves = build_metric_curves(obj, cycle_indices, builder=build_qd_vs_voltage_curve)
            if curves:
    ...
    curves = build_metric_curves(chosen_obj, cycle_indices, builder=build_qd_vs_voltage_curve)
    plot_three_panel_metric(
        curves,
