...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot current vs time, voltage vs time, and dQ/dV vs voltage for the CHARGE period."
    )
    ...
    parser.add_argument("--min-dv", type=float, default=1e-4, help="Minimum |delta voltage| for dQ/dV")
    ...
    if all_files_mode:
        ...
        for subfolder_name, paths in subfolder_groups:
            ...
            for path in paths:
                ...
                curves = build_charge_metric_curves(
                    obj, indices,
                    builder=lambda series: build_dqdv_curve(series, min_dv=args.min_dv),
        ...
        plot_three_panel_metric_dashboard(
            curves_by_subfolder,
            metric_title="dQ/dV vs Voltage (Charge)",
            metric_ylabel="dQ/dV (Ah/V)",
