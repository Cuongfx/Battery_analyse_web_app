...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot maximum charge capacity (Qcmax) vs cycle from a BatteryML .pkl file."
    )
    ...
    if all_files_mode:
        ...
        for subfolder_name, paths in subfolder_groups_raw:
            ...
            for path in paths:
                try:
                    ...
                    cycle_axis, qcmax_values = compute_qcmax_by_cycle(obj)
                    file_entries.append((path.stem, cycle_axis, qcmax_values))
        ...
        plot_qdmax_all_files_dashboard(
            subfolder_groups,
            title="Qcmax vs Cycle  |  All Subfolders — All Files",
    ...
    if combined_mode:
        ...
        for path in chosen_files:
            obj = load_batteryml_pickle(path)
            cycle_axis, qcmax_values = compute_qcmax_by_cycle(obj)
            datasets.append((path.parent.name, cycle_axis, qcmax_values))
        plot_qdmax_dashboard(
            datasets=datasets,
            title="Qcmax vs Cycle  |  All Selected Subfolders",
            axis_range=axis_range,
            save_path=save_path,
            ylabel="Qcmax (Ah)",
        )
