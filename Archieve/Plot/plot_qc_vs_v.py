...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot current vs time, voltage vs time, and charge capacity vs voltage from a BatteryML .pkl file."
    )
    ...
    if all_files_mode:
        ...
        for subfolder_name, paths in subfolder_groups:
            ...
            for path in paths:
                ...
                curves = build_charge_metric_curves(
                    obj, indices,
                    builder=build_qd_vs_voltage_curve,
        ...
        plot_three_panel_metric_dashboard(
            curves_by_subfolder,
            metric_title="Qcharge vs Voltage",
            metric_ylabel="Qcharge (Ah)",
    ...
    if combined_mode:
        ...
        for path, obj in objects:
            curves = build_charge_metric_curves(obj, cycle_indices, builder=build_qd_vs_voltage_curve)
            if curves:
    ...
    curves = build_charge_metric_curves(chosen_obj, cycle_indices, builder=build_qd_vs_voltage_curve)
    plot_three_panel_metric(
        curves,
